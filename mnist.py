...
class EOIntensityModulation(tf.keras.layers.Layer):
    ...
    def __init__(self,
                 ...
                 single_param_per_layer=True, **kwargs):
        ...
        super(EOIntensityModulation, self).__init__( **kwargs)
        ...
        self.g = self.add_weight(shape=var_shape,

                                   name="g",

                                   initializer=tk.initializers.Constant(g),

                                   trainable=train_g,

                                   constraint=lambda x: tk.backend.clip(x, 1e-3, 1.5 * np.pi))

        self.phi_b = self.add_weight(shape=var_shape,

                                       name="phi_b",

                                       initializer=tk.initializers.Constant(phi_b),

                                       trainable=train_phi_b,

                                       constraint=lambda x: tk.backend.clip(x, -np.pi, +np.pi))

        self.alpha = self.add_weight(shape=var_shape,

                                       name="alpha",

                                       initializer=tk.initializers.Constant(alpha), #tf.constant_initializer

                                       trainable=train_alpha,

                                       constraint=lambda x: tk.backend.clip(x, 0.01, 0.99)) #tf.clip_by_value

        # By Zhenyu ZHAO
        self.N = N
        self.train_alpha = train_alpha
        self.train_g = train_g
        self.train_phi_b = train_phi_b
        self.single_param_per_layer = single_param_per_layer
    # By Zhenyu ZHAO
    def get_config(self):
        config = super(EOIntensityModulation, self).get_config().copy()
        # config.update({
        #     'g': self.g,
        #     'phi_b': self.phi_b,
        #     'alpha': self.alpha,
        ...
        #     'train_g': self.train_g,
        #     'train_phi_b': self.train_phi_b,
        #     'single_param_per_layer': self.single_param_per_layer,
        # })
        return config
    ...
    def call(self, inputs, **kwargs):
        ...
        return 1j * tf.sqrt(1 - alpha) * tf.exp(-1j * 0.5 * g * tf.math.conj(Z) * Z - 1j * 0.5 * phi_b) * tf.cos(
            0.5 * g * tf.math.conj(Z) * Z + 0.5 * phi_b) * Z
...
ONNData = namedtuple('ONNData',
                     ['x_train', 'y_train', 'y_train_ind', 'x_test', 'y_test', 'y_test_ind', 'units', 'num_classes'])

# ref : https://blog.csdn.net/kane7csdn/article/details/83928848
...
# @ Zhenyu ZHAO
def get_left_up_triangle_2D(arr):
    column = arr.shape[1]
    flip_arr = flip90_right(arr)
    new_arr = np.array([])
    for i in range(0, column):
        new_arr = np.append(new_arr,np.diag(flip_arr, i))
    return new_arr.reshape(1,-1)

# @ Zhenyu ZHAO
def cal_units(length):
    series = 0
...
class MNISTDataProcessor:
    ...
    def __init__(self, fashion: bool = False):
        ...
        self.x_test_raw, self.y_test) = fashion_mnist.load_data() if fashion else mnist.load_data()
        ...
        self.ones_zeros_array = np.zeros((28, 28), dtype=float)
    ...
    def resample(self, p, b=0): ##图像旋转
        ...
        ## Zhenyu ZHAO 20200721
    def DCT(self, remain_length):
        for i in range(0, remain_length):
            for j in range(0,remain_length):
                self.ones_zeros_array[i][j] = 1

        x_train_dct = []
        x_test_dct = []
        for pic in self.x_train_raw:
            x_train_dct.append(get_left_up_triangle_2D((self.ones_zeros_array * cv2.dct(pic.astype('float')).real)[0:remain_length, 0:remain_length]))

        for pic_test in self.x_test_raw:
            x_test_dct.append(get_left_up_triangle_2D((self.ones_zeros_array * cv2.dct(pic_test.astype('float')).real)[0:remain_length, 0:remain_length]))

        x_train_dct = np.asarray(x_train_dct)
        x_test_dct = np.asarray(x_test_dct)
        return ONNData(

            x_train=norm_inputs(x_train_dct.reshape((self.num_train, -1))).astype(np.complex64),

            y_train=np.eye(10)[self.y_train],

            y_train_ind=self.y_train,

            x_test=norm_inputs(x_test_dct.reshape((self.num_test, -1))).astype(np.complex64),

            y_test=np.eye(10)[self.y_test],

            y_test_ind=self.y_test,

            units=cal_units(remain_length),
